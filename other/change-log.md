...
* **Lacuna Diamond** теперь можно активировать за токены, которые можно получить голосуя за бота на платформе [top.gg](https://top.gg/bot/740585412560420914/vote)
  * Активировать Lacuna Diamond за Nitro Boost теперь нельзя, если сервер поддержки имеет более 21 буста
* **Логи**: В сообщениях теперь дополнительно отображается упоминание пользователя, который связан с событием
  * Событие "Сообщение изменено" теперь отображает, что именно было изменено в сообщении
* Команды `reason` и `poll` были удалены из-за неактуальности
* Таблицу лидеров теперь можно просмотреть на сайте
* "Роли экспертов" теперь называются "Эксперты по боту" и могут содержать идентификаторы пользователей
* Исправлена проблема из-за которой не работало событие "Голосовое отключение" в автоматизации
